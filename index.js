...
document.addEventListener("click", closeAllSelect);
  ...
  document.getElementById("getColorBtn").addEventListener("click", function() {
    const colorValue = (document.getElementById("color-picker").value).replace("#", "");
    const colorTypeValue = document.getElementById("color-type").value;


  fetch(`https://www.thecolorapi.com/scheme?hex=${colorValue}&mode=${colorTypeValue}&count=6`)
    .then(response => response.json())
    .then(data => {
      document.getElementById("colorPalet").innerHTML = `
      ...
      <div style="background-color: ${data.colors[5].hex.value};"></div>
      `
      document.getElementById("colorHexCodes").innerHTML = `
      ...
      <div>${data.colors[5].hex.value}</div>
      `
